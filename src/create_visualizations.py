...
def plot_weather_scatter(df: pd.DataFrame, outdir: str) -> str:
    ...
    precip_vals = np.linspace(0.0, max_precip, 4)
    precip_handles = []
    for val in precip_vals:
        size = np.interp(val, [0.0, max_precip], size_range)
        handle = plt.scatter([], [], s=size, c='gray', alpha=0.6, label=f"{val:.2f}")
        precip_handles.append(handle)

    precip_legend = plt.legend(
    ...
    # Restore the first legend (city_legend) to the plot
...
def plot_global_temp_heatmap(df: pd.DataFrame, outdir: str) -> str:
    ...
    plt.xticks(ticks=np.arange(12) + 0.5, labels=months, rotation=45)
